...
前一轮文档发布：`324b7a796f9eb0115555242d7ad6fb04fde3c546`，分支 `feature/reconfigurable-navigation-oracle`；其父提交 `008a7958cf542fe05a318290d4534358e0704f2b` 是箱体技能训练代码发布。本文件描述随后完成的executor/快照集成；当前发布提交以实际Git历史为准，运行源码以记录中的SHA-256为准。
...
最新用户指令：“先接受技能成功率，利用现有技能将项目往前推进”。本轮已将冻结技能接入共享 `ReconfigurableExecutor`、专用规则 `BoxSupportPlanner` 和N1记录/快照。正式技能序列为 `PUSH -> NAV -> CLIMB -> NAV -> CLIMB -> STOP`，每个技能后重新观察并规划。成功seed500、509和已知失败510的阶段报告、耗时及末状态与原流程完全一致；这属于已知种子的集成回归，不是新的独立成功率估计。

新增数据有10条转换及10个完整快照，覆盖成功509和失败510；十条记录均跨进程零容差重放通过，包含高台失败。候选工具兼容检查为12请求：3成功、1物理失败、8前置拒绝。当前后端只接受已验证的支撑目标，普通参数扰动被拒绝，尚未完成几何候选搜索或世界模型训练。实现、复现与产物见 [移动支撑执行与数据](BOX_SUPPORT_EXECUTOR_CN.md)。
...
当前下一步：利用已冻结的技能扩展推面、接近、支撑停放与落点的几何候选，采集按场景族划分的真实转移数据，再训练特权状态世界模型并接入候选选择。继续提高低层成功率不再是前置条件；3次高台接触失败保留为已知边界。VLM、RGB-D、JUMP和实机迁移后置，新候选与旧默认策略不得混用。
...
| 下一项开发 | 几何候选、按场景族划分的转移数据、特权状态世界模型 |

## 3. 用户已经确认的方向

- 最新优先级是接受29/32组合技能并利用现有技能推进项目；不继续把低层可靠性专项作为数据与模型工作的前置条件。单独actor、组合流程和旧基线的成绩分别记录。

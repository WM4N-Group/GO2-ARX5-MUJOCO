...
目前 CLIMB 的箱体是地形网格中的固定支撑面。PUSH 使用可移动箱，但两者尚未在同一个可移动支撑任务中联合验收。不得将固定箱登顶等同于已完成推动轻箱后登箱的动力学。
...
同目录 `continuous-climb-validation-v2.json` 是本轮最终索引，绑定三份选用策略、各包manifest、两台机器的报告和录像hash，并记录29/32成功率及3次高台机身接触。索引明确高台actor的原生独立验收未通过、尚未接入生产executor、尚未消除全部失败。原来的 `mujoco-deployment-validation.json` 保留为上一阶段快照。

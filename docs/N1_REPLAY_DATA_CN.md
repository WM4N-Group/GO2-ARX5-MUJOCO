...
范围说明（2026-09-15）：本文保留N1旧生产技能快照与pilot的阶段记录，功能随后发布于f857437并由008a795代码基线继承。新移动箱组合现已接入同一记录/快照流程，本轮10条记录跨进程零容差重放通过，见 [Box执行与数据记录](BOX_SUPPORT_EXECUTOR_CN.md)。旧pilot与新Box数据的工况和指标分别记录；历史数据须使用对应代码指纹，不能修改manifest绕过兼容检查。当前优先级以 [主交接](AGENT_HANDOFF_CN.md) 为准。

后续新Box场景的采集器已按快照观测生成有界停车、接近和落点；42请求得到29成功、7物理失败、6非法请求拒绝，见 [几何候选pilot](BOX_SUPPORT_GEOMETRY_CN.md)。其他场景仍使用原局部扰动生成器。全部候选共享其来源起点，不能作为独立场景拆分到训练与测试两侧。

...
def load_dataset(manifest_paths):
    ...
    for manifest_path in map(Path, manifest_paths):
        ...
        for candidate in candidates:
            ...
            for name in ("dataset_split", "scene_family", "scene_id", "episode_id", "snapshot_sha256"):
                if candidate[name] != origin[name]:
                    raise ValueError(f"Candidate/source mismatch: {name}")
            for name in ("scene_family", "scene_id", "episode_id", "snapshot_sha256", "split_group_id"):
            ...
            if encoded is not None:
                ...
                metadata.append({"candidate_id": candidate["candidate_id"], "split": split, "skill": action["skill"], "anchor": anchor, "scene_id": candidate["scene_id"], "episode_id": candidate["episode_id"], "snapshot_sha256": candidate["snapshot_sha256"]})

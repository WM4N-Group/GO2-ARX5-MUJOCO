...
- The current planner is a privileged-state, rule-based Oracle. A first offline pose-transition MLP has been trained; it did not beat the conditional baseline on pose accuracy or candidate selection and must not replace the planner. The target BEV/object Transformer ensemble, VLM proposals, and RGB-D inference are still planned. See [the model framework and baseline](docs/WORLD_MODEL_BASELINE_CN.md).
